...
namespace Tekstorm
{
	namespace IO
	{
		const int32_t ORIGIN_START = 0;
		const int32_t ORIGIN_CURRENT = 1;
		const int32_t ORIGIN_END = 2;
		...
		public:
			...
			///
			virtual void CopyTo(IStream &destination, int64_t count) = 0;
			...
			///
			virtual int64_t Read(char *buffer, int64_t offset, int64_t count) = 0;
			...
			///
			virtual void Write(const char *buffer, int64_t offset, int64_t count) = 0;
